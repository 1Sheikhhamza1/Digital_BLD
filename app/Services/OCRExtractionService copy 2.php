...
class OCRExtractionService
{
    ...
    protected function extractContentFromJudgment(string $text): ?string
    {
        // Match "Judgment" as a standalone line
        if (preg_match('/^\s*Judgment\s*:?\s*$/im', $text, $m, PREG_OFFSET_CAPTURE)) {
            $startPos = $m[0][1] + strlen($m[0][0]);

            // Get everything after "Judgment"
            $afterJudgment = substr($text, $startPos);

            // Stop at the next heading if found
            if (preg_match('/^\s*(Order|Result|Conclusion)\b/im', $afterJudgment, $stopMatch, PREG_OFFSET_CAPTURE)) {
                $afterJudgment = substr($afterJudgment, 0, $stopMatch[0][1]);
            }

            return trim($afterJudgment);
        }
