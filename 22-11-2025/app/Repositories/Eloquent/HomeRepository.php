...
class HomeRepository implements HomeRepositoryInterface
{
    ...
    {
        return OCRExtraction::whereNotNull('volume_id')->where('homepage', 1)
            ->orderBy('id', "DESC")
            ->limit(6)
            ->get();
    }
    ...
    public function getHowToBldWorks()
    {
        return Page::with('children')->where(function ($q) {
            $q->where('slug', 'how-to-bld-works')
                ->orWhere('id', 42);
        })
            ->where('status', 1)
            ->orderBy('sequence')
    ...
    public function getPagesBySlugs(string $slug, ?string $sslug = null): array
    {
        ...
        if ($sslug === null) {
            $othersPages = Page::select('title', 'slug')->whereNull('parent_id')->where('page_structure', 'Text')->get();
    ...
    // Package
    public function getPackage($limit = 10)
    {
        $query = Package::with(['features' => function ($q) {
            $q->where('status', 1)->whereNull('deleted_at');
        }, 'features.modules' => function ($q) {
            $q->where('status', 1)->whereNull('deleted_at');
        }])
            ->where('status', 1)
            ->latest();
    ...
    public function getFaq($limit)
    {
        if (isset($limit) && $limit == 'all') {
            return Faq::where('status', 1)->orderBy('sequence', 'ASC')->get();
        } else {
            return Faq::where('status', 1)->orderBy('sequence', 'ASC')->paginate($limit);
        }
    }


    public function getUserManual($limit, $keywords = null)
    {
        // Base query
        $query = UserManual::where('status', 1);

        // Apply keywords search if provided
        if (!empty($keywords)) {
        ...
        // Return paginated results
            ...
            $result = $query->orderBy('sequence', 'ASC')->paginate($limit);
        }
