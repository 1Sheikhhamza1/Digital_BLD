        ...
        <div class="row g-3">
            ...
            <div class="col-6 col-md-4 col-lg-3">
                <a class="card-item" href="{{ route('subscriber.sharedDecision', Crypt::encrypt($share->id)) }}">
                    <h5 class="card-title">
                        {{ $share->decision->parties ?? 'Untitled Case' }}
                        ({{ $share->decision->decided_on ?? 'N/A' }} - {{ $share->decision->division ?? 'N/A' }})
                    </h5>
                    <p class="card-text">
                        {!! Str::limit(strip_tags($share->decision->judgment) ?? 'No description available.', 250) !!}
                    </p>
                </a>
            </div>
            @empty
