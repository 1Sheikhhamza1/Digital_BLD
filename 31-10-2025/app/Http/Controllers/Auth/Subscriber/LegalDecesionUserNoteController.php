...
class LegalDecesionUserNoteController extends BaseController
{
    ...
    public function editDecisionNote($id)
    {

        $myNotes = LegalDecisionUserNote::where([['user_id', $this->subscriberId], ['id', $id]])->first();
        return view('auth.subscribers.profile.edit_decision_note', compact('myNotes'));
    }
    ...
    public function updateDecisionNote(Request $request, $id)
    {
        ...
        return redirect()->route('subscriber.myDecision', Crypt::encrypt($id))->with('success', 'Your note has been saved!');
    }
